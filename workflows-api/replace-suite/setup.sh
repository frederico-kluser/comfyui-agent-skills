#!/usr/bin/env bash
# setup.sh — replace-suite (roupa + fundo + pose num arquivo só; rode UM processo por vez, via API)
...
NODES=(
  ...
  "https://github.com/kijai/ComfyUI-KJNodes"      # ImageResizeKJ (nas variantes Nano)
)
WORKFLOWS=(
  "00_LEIA-ME_comece_aqui.json" "10_modular_nanobanana.json" "11_modular_kontext.json"
)
...
echo "============================================"
echo " replace-suite pronto. Reinicie o ComfyUI."
echo " Nós vermelhos? Manager > Install Missing (fal-API, KJNodes)."
echo " Rode UM processo por vez: ative o 'Salvar' do bloco desejado (Ctrl+B) e deixe os outros em bypass."
echo " Cada etapa: grupo Referência em bypass = TEXTO; ative + suba a foto = REFERÊNCIA."
