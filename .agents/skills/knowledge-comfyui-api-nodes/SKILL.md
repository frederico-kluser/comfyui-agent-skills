...
**Música (text-to-music):** nós **partner clicáveis existem** — 🏆 **`StabilityTextToAudio`** (Stable Audio 2.5, créditos comfy.org; a ToS **hospedada** da Stability **§4.a CEDE a posse** do output e sobrevive ao cancelamento §12.e → comercial-limpo; o teto US$1M/revogável é só dos **pesos self-hosted**, não da API) · `SoniloTextToMusic` (Sonilo — comercial só no tier pago, **sem cláusula de sobrevivência** → atrelado à assinatura) · `Replicate meta/musicgen` (`comfyui-replicate`) = **CC-BY-NC NÃO-comercial** ⛔. A licença mais limpa segue sendo **ACE-Step** por **script** (fal `fal-ai/ace-step` → `audio.url` WAV; Replicate `fishaudio/ace-step-1.5`, ~US$0,095/faixa) **ou** pelo grafo **core local** (`EmptyAceStepLatentAudio`→`TextEncodeAceStepAudio`→`KSampler` euler/simple/50/cfg5 + `ModelSamplingSD3` shift 5→`VAEDecodeAudio`→`SaveAudio`; checkpoint `ace_step_v1_3.5b.safetensors`, ~8GB VRAM, **sem custom node**). Instrumental = `lyrics` `[inst]`/`[instrumental]`. **Licença é o que decide o provedor:** ACE-Step **1.5=MIT / v1=Apache-2.0** → comercial perpétuo/irrevogável; no **Replicate** a ToS **dá posse do output e sobrevive ao cancelamento** (§5+§9.5) → melhor p/ vender "para sempre". 🔴 **Suno/Udio** = em litígio (Sony/UMG/Warner); ⛔ **Mubert/Beatoven** = direito atrelado à assinatura ativa. ⚠️ **créditos comfy.org NÃO dão direito por si**: a ToS do comfy.org define "Output" só como conteúdo **visual** → áudio de partner é regido pela ToS do **provedor** (por isso o Stability, que cede posse, é o que vale). Template pronto: `api_stability_ai_text_to_audio.json`. Bundle pronto: `workflows-api/text-to-music-api/`. Formato p/ loop: **WAV/FLAC/OGG, nunca MP3** (padding de encoder quebra o loop).
